...
tests =
  [ testGroup
      "=G= list2cons Function"
      [ testProperty
          "=P= Converts [a] to (List a) exactly (5 points)"
          (prop_list2cons_eq_foldableToCons :: [Integer] -> Property)
      ]
  , testGroup
      "=G= cons2list Function"
      [ testProperty
          "=P= Converts (List a) to [a] exactly (5 points)"
          (prop_cons2list_eq_foldableToList :: List Integer -> Property)
      ]
  , testGroup
      "=G= eval Function"
      [ testProperty
          "=P= Evaluates expressions correctly (5 points)"
  ...
  , testGroup
      "=G= sumTree Function"
      [ testProperty
          "=P= Sums all values in the tree (5 points)"
          (prop_sumTree_eq_foldableSum :: BinTree Integer -> Property)
      ]
  , testGroup
      "=G= liftIntOp Function"
      [ testProperty
          "=P= Lifts integer operations correctly (5 points)"
