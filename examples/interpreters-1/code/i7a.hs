...
letExp = do symbol "let"
            symbol "["
            locals <- many1 pairAsn
            symbol "]"
            symbol "in"
            e2 <- expr
            symbol "end"
            return $ LetExp locals e2
...
eval (LetExp locals e2) env =
  let vals = map (\(v,e) -> (v,eval e env)) locals
   in eval e2 (vals ++ env)
...
eval (AppExp e1 e2) env =
  ...
  in case v of
     [thev] -> eval e3 ((thev,arg):cenv)
     (x:xs) -> ClosureVal xs e3 ((x,arg):cenv)
